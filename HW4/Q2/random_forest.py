...
class RandomForest(object):
    ...
    def _bootstrapping(self, XY, n):
        '''
        Summary: Construct a dataset from XY by sampling with replacement
        XY: A list of lists or numpy array of dimension (m,n)
        n: Size of the dataset
        sample_X: Numpy array of dimension (m,n-1)
        sample_Y: Numpy array of dimension (m,)
        '''
        ...
        sample_row_idx = np.random.randint(0, num_row, size=(n,))
        sample_X = XY[sample_row_idx, :-1]
        sample_Y = XY[sample_row_idx, -1]
        return sample_X, sample_Y

    def bootstrapping(self, XY):
        '''
        Summary: Initializing the bootstap datasets for each tree
        '''
        for i in range(self.num_trees):
            data_sample, data_label = self._bootstrapping(XY, len(XY))
    ...
    def fitting(self):
        ...
        '''
        Summary: Train decision trees with the bootstraps datasets
                 using random subspace method
        '''
        num_col = self.bootstraps_datasets[0].shape[1]
        num_subattr = int(np.sqrt(num_col))
        for i, DT in enumerate(self.decision_trees):
            print 'Training the decision tree: {}'.format(i+1)
            X = self.bootstraps_datasets[i]
            y = self.bootstraps_labels[i]

            col_idx = np.random.permutation(num_col)[:num_subattr]
            subX = X[:, col_idx]
    ...
    def voting(self, X):
        '''
        Summary: Give the final prediction for X based on majority vote
        X: A list of lists
        y: A list of predictions
        '''
        y = []
        in_bag_count = 0
        for record in X:
        ...
        print 'Number of in-bag records: {}'.format(in_bag_count)
        return y
